...
Source plan: `docs/plans/2026-07-24-public-core-internal-integration-minimization.md`

Control plane: root intuitive-flow session / host goal
`019f9981-2c0a-79e0-969b-38876bfa3a74`
...
after disk capacity and EULA readiness are resolved.

Stop condition: stop for a material boundary change, overlapping concurrent edits, publication or
history mutation, unavailable required live provider/hardware proof, or any physical safety gate.

No-touch scope: NVIDIA CUDA/RTX/Isaac runtime, unrelated product semantics/scoring,
`job_config_template.yaml`, public push, history rewriting, private asset redistribution, and
physical movement.

Current blockers: `just dev::network-status` reports `network: unknown` because its probe URL is
missing. Kimi and MiniMax configuration is present, but their live calls remain fail-closed; all
three custom Responses values are missing. Agibot and B1 injected dependency paths are missing,
Agibot physical movement remains unauthorized, and Isaac strict preflight reports 17.0 GiB free
versus 80 GiB required plus unaccepted EULA terms.
