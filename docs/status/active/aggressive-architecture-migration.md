...
- Current slice: Wave 1 household runtime/realworld initialization ownership
- Last proven evidence: interactions/state SCC absent; full operator-console suite, host console HTTP inventory smoke, architecture ratchet, Ruff/format, stale-import, and diff gates pass
- Completed batch: Wave 0 complete; five Wave 1 SCC slices reduce the graph from six SCCs to one without compatibility facades
- Next slice: remove the function-local `realworld_contract_init` back-import from `household_runtime_contract` through one narrow runtime initialization owner
- Next proof: household runtime initialization and CLI contracts, zero-SCC architecture ratchet, Ruff/format, stale-import absence, and `git diff --check`
