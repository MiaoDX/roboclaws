...
- **Residual eval findings:** One missing historical fixture remains, and the latest full baseline refresh has Kimi cleanup comparison trials blocked by the provider's 5-hour quota; no durable baseline was published.
...
and the latest Kimi cleanup comparison is provider-quota blocked. Addressing
the fixture or rerunning the blocked comparison requires
